...
function UserTable({ users, handleEdit, handleDelete, handleSort, getSortIconClass }) {
  return (
            ...
            <table className="min-w-full text-left text-md font-light text-surface dark:text-black">
              <thead className="border-b border-neutral-200 bg-neutral-100 font-medium/10">
                <tr>
                  ...
                  >
                    Status Keanggotaan {getSortIconClass('status')}
                  </th>
                ...
                {users.map((user, index) => (
                  <tr key={index} className="border-b border-neutral-200">
                    ...
                    <td className="whitespace-nowrap px-6 py-4">
                      <span className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${user.status === 'active' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
